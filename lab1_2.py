...
def read_and_analyze_file(path_to_dataset):
    """
    ...
    result_lst = []
    needed_info = []
    with open('locations.list', 'r') as file:
        for line in file:
...
def analyze_the_coordinates(resulted_dict, year, latitude, longitude):
    """
        ...
        if element == year:
            for item in resulted_dict[element]:
                try:
                    ...
                    location = geolocator.geocode(geo_locate)
    ...
    for item in list_of_coords:
        distance = geodesic(init_coord, item).km
...
def generate_map(main_dict, latitude, longitude, year, path_to_dataset):
    ...
    """
    map = folium.Map(tiles="Stamen Terrain", location=[latitude, longitude], zoom_start=15)
    ...
    for item in main_dict:
        optional_latitude = main_dict[item][0][1][0]
